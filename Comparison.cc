...
bool Comparison :: Run (Record& left, Record& right) {
	...
	//cout <<"Left_bits: "<< left_bits<<endl;
	//cout << "right_bits: "<<lit_bits<<endl;
	...
	switch (attType) {
		...
		case Float: {
			//cout << "val1Double: "<<val1Double<<endl;
			//cout << "val1: "<<*val1<<endl;
			//if(*val1 == '(') break;
...
int CNF::ExtractCNF (AndList& parseTree, Schema& schema, Record& literal) {
	...
	while (currCond != NULL) {
		...
		if (currCond->left->left->code == NAME) {
			...
			attStart[numFieldsInLiteral] = recSize;
			int cLen = sizeof(int);
			int val = atoi (currCond->left->left->value);
			memcpy(recPos, &val, sizeof(int));
			recSize += cLen;
			recPos += cLen;
			numFieldsInLiteral += 1;
		}
			...
			double val = atof (currCond->left->left->value);
			memcpy(recPos, &val, sizeof(double));
			recSize += cLen;
			recPos += cLen;
			numFieldsInLiteral += 1;
		}
		...
		if (currCond->left->right->code == NAME) {
			...
			attStart[numFieldsInLiteral] = recSize;
			int cLen = sizeof(int);
			int val = atoi (currCond->left->right->value);
			memcpy(recPos, &val, sizeof(int));
			recSize += cLen;
			recPos += cLen;
			numFieldsInLiteral += 1;
			...
			attStart[numFieldsInLiteral] = recSize;
			int cLen = sizeof(double);
			double val = atof (currCond->left->right->value);
			memcpy(recPos, &val, sizeof(double));
			recSize += cLen;
			recPos += cLen;
			numFieldsInLiteral += 1;
